      ...
      <el-card class="box-card">
        <el-form ref="form" :model="form" label-width="120px">
          ...
          <el-form-item label="Access Key">
            <el-tooltip class="item" effect="dark" content="(Recommended) Only people with the correct key can register" placement="right">
          ...
          <el-form-item label="Rewards">
            ...
            <el-button type="success" class="mtop" @click="addReward = true">Add Reward</el-button>
          </el-form-item>
          <el-form-item>
            <el-button type="primary" @click="updateFirebase">Save</el-button>
          </el-form-item>
        </el-form>
      </el-card>
...
export default {
  ...
  methods: {
    updateFirebase () {
      this.$store.dispatch('updateFirebase', this.form)
      this.$message({
    ...
    setReward () {
      this.form.rewards.push([0, this.rewardsForm])
      this.rewardsForm = {
        costInPoints: 1000,
        rewardName: '',
        imageURL: '',
  ...
  computed: {
    ...
    masterObj () {
      return this.$store.getters.masterObj
    },
    tableData () {
      if (this.masterObj != null) {
        return this.masterObj.rewards.map(obj => {
          return {
            Reward: obj[1].rewardName,
            Cost: obj[1].costInPoints
          }
        })
      }
    }
  ...
  created () {
    if (this.user === null) {
      ...
      this.$store.dispatch('populateMasterObj')
      if (this.masterObj !== null) {
        this.form = this.masterObj
      }
    }
  },
  watch: {
